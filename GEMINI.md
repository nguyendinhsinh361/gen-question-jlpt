...
- Character counting (700-800 chars cho N1, 700-770 cho N2, 600-750 cho N3, 400-500 cho N4, 250-290 cho N5). Hard reject nếu dưới Min (không được nhỏ hơn 100% minimum).
...
- **NGHIÊM CẤM che khuất chữ tiếng Nhật**: Mọi ký tự tiếng Nhật phải hiển thị rõ ràng 100%, KHÔNG bị đè/che bởi bất kỳ element nào — bao gồm floating label, step badge, box border, heading background, **hình vẽ, icon, emoji, SVG**. Khi dùng floating label (`position: absolute`): box phải có `position: relative`, `margin-top ≥ 16px`, `padding-top ≥ 24px`, label có `background-color` solid. Khi dùng icon/hình trang trí: KHÔNG đặt chồng lên text, tách riêng vùng hình và chữ. **Nguyên tắc: nếu không thể hiển thị cả hình VÀ chữ rõ ràng → BỎ HÌNH, GIỮ CHỮ.**
