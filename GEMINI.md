...
- **NGHIÊM CẤM che khuất chữ tiếng Nhật**: Mọi ký tự tiếng Nhật phải hiển thị rõ ràng 100%, KHÔNG bị đè/che bởi bất kỳ element trang trí nào (floating label, step badge, box border, heading background). Khi dùng floating label (`position: absolute`): box phải có `position: relative`, `margin-top ≥ 16px` (tránh che text phía trên), `padding-top ≥ 24px` (tránh che text bên trong), label phải có `background-color` solid.
