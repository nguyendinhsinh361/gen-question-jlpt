...
2. **Screenshot PNG** → `assets/img/tim_thong_tin/{LEVEL}_{uuid}.png`
   Captured from the HTML via Playwright. Automatically uploaded to Google Drive; the Drive direct link is stored in CSV column `general_image`.
   Example: `assets/img/tim_thong_tin/N3_a1b2c3d4.png` → CSV stores `https://drive.google.com/uc?id=FILE_ID&export=view`
...
| `general_image` | Google Drive direct link: `https://drive.google.com/uc?id=FILE_ID&export=view` — auto-uploaded from `assets/img/tim_thong_tin/{LEVEL}_{uuid}.png` |
...
9. **Upload images to Google Drive** → run `scripts/upload_to_drive.py` to upload PNGs to Drive and get direct links. This step is **mandatory** — generating data always includes uploading.
10. **Extract clean HTML** → build CSV rows
11. **Generate questions** → create questions, answer options, correct answers, and explanations per the rules above
12. **Fill CSV columns** → `tag` = format label, `general_image` = Drive direct link from step 9, `question_label_{i}`, `question_{i}`, `answer_{i}`, `correct_answer_{i}`, `explain_vn_{i}`, `explain_en_{i}`
13. **Append to CSV** (or create new if starting fresh)
14. **Verify** all character counts, question counts, format diversity, and file integrity
...
### upload_to_drive.py — Upload images to Google Drive & update CSV

`scripts/upload_to_drive.py` uploads screenshot PNGs to Google Drive and replaces local paths in the CSV `general_image` column with Drive direct links.

**Credentials location:** `secrets/` folder at project root (auto-detected):

```
project-root/
├── secrets/
│   ├── service_account.json   ← Option A (recommended)
│   ├── credentials.json       ← Option B (OAuth2)
│   └── token.json             ← auto-generated after OAuth2 auth
```

**Option A — Service Account (recommended, no browser needed):**
1. Google Cloud Console → IAM & Admin → Service Accounts → Create
2. Download JSON key → save as `secrets/service_account.json`
3. Share Drive folder `13hW7nalQDbpL8olH4ffRApvHc-24Fzr3` with service account email (Editor role)
4. Done! No `--auth` step needed.

**Option B — OAuth2 (needs browser once):**
1. Google Cloud Console → APIs & Credentials → Create OAuth 2.0 Client ID (Desktop)
2. Download JSON → save as `secrets/credentials.json`
3. Run: `python3 <skill>/scripts/upload_to_drive.py --auth`
4. Browser opens → authorize → `secrets/token.json` saved automatically

**Upload & update CSV** (after every gen batch):
```bash
# Upload all images + update CSV (credentials auto-detected from secrets/)
python3 <skill>/scripts/upload_to_drive.py \
    --img-dir assets/img/tim_thong_tin \
    --csv sheets/n5_samples_v2.csv

# Upload specific files only
python3 <skill>/scripts/upload_to_drive.py \
    --files N5_a1b2c3d4.png N5_e5f6g7h8.png \
    --img-dir assets/img/tim_thong_tin \
    --csv sheets/n5_samples_v2.csv
```

The script will:
- Auto-detect auth method (service_account.json > credentials.json > error with guide)
- Upload PNG files to the default Drive folder (`13hW7nalQDbpL8olH4ffRApvHc-24Fzr3`)
- Set permissions to "anyone with link can view"
- Replace `assets/img/tim_thong_tin/N5_a1b2c3d4.png` → `https://drive.google.com/uc?id=FILE_ID&export=view` in CSV
- Save a `_drive_links.json` mapping file for reference
- If a file already exists in Drive, it updates instead of duplicating
